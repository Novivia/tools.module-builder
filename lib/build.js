...
export class Build {
  ...
  async compileBabelFile(filePath: string) {
    ...
    if (isMatch) {
      ...
      if (this.useBabelRuntime) {
        options.plugins.push(require("babel-plugin-transform-runtime"));
      }
  ...
  async compileBabelFiles() {
    ...
    await new Promise((resolve, reject) => {
      const self = this;
      const walker = walk(this.basePath, function (path) {
        if (basename(path) === "node_modules") {
          // Don't ever compile files located in the "node_modules" directory.
          self.logger.warn(noMultiSpaceAfterLineFeed`
            You tried to compile files located in the "node_modules" directory,
            they were omitted.
          `);
