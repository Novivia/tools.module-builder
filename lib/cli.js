...
console.log("user package info:", userPackageInfo);
...
async function buildCommandHandler(argv) {
  ...
  if (argv.package) {
    packagePatterns.concat(argv.package);
  }

  const babelPatterns = userPackageInfo.babelPatterns || [];
  if (argv.babel) {
    babelPatterns.concat(argv.babel);
  }

  let compileTargets = userPackageInfo.compileTargets;
  if (argv.nodeTarget) {
    compileTargets = {node: argv.nodeTarget};
  }
